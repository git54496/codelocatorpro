...
2. 执行 grab CLI（优先 live，失败回退 file）
   - 二进制：`adapter/build/install/codelocator-adapter/bin/codelocator-adapter`
   - 构建（若二进制不存在）：
     - `cd adapter && ./gradlew installDist --no-daemon`
   - 抓取：
     - `codelocator-adapter grab live --json`
     - `codelocator-adapter grab file --json`（live 失败时）
   - 从 JSON 输出提取 `grabId`。
